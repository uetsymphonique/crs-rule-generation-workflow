# run-pipeline.ps1 - batch CRS rule-generation pipeline
#
# Usage (from repo root):
#   .\run-pipeline.ps1 -List cve-list.txt
#   .\run-pipeline.ps1 -List cve-list.txt -GenVariants root-cause-only
#   .\run-pipeline.ps1 -List cve-list.txt -Resume          # skip IDs that already have output
#   .\run-pipeline.ps1 -List cve-list.txt -MaxBudgetUsd 2.00 -MaxTurns 20
#
...
Log "=== pipeline start: $($templates.Count) template(s), model=$Model, gen-variants=$GenVariants, max-budget=`$$MaxBudgetUsd/step, max-turns=$MaxTurns ==="
...
Log ("=== pipeline end: ok={0} covered={1} warn={2} | tokens in={3} out={4} cache_read={5} cache_write={6} total_cost=`${7:F4} ===" `
