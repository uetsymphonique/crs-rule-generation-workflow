...
description: Stage 2 primary của pipeline CRS rule-generation (Action B). Nhận verdict not-covered + Nuclei template + extended-requests (PoC + biến thể từ crs-variant-gen), synthesize MỘT CRS SecRule mới từ zero với RAG từ comibined-docs. Thiết kế variable scope, operator, transform pipeline, phase, anomaly scoring; chọn rule ID không conflict; bắt attack class bao phủ cả PoC lẫn extended variants. Verify bằng engine (verify_rule.py) trước khi emit — max 3 vòng. Write-only → out/<id>/new.json. KHÔNG sửa rule có sẵn (đó là crs-rule-fix), KHÔNG commit vào coreruleset.
...
```
INGEST ──► DESIGN ──► SYNTHESIZE ──► VERIFY ──┬─ all triggered ──────────────────────► EMIT ──► STOP
   │                      ▲                   │─ parse_ok=false ─► SYNTHESIZE (fix syntax, loop, iter không tăng)
   │                      └── metadata fail ──┘─ any triggered=false & iter < 3 ─► DESIGN (loop, iter++)
   └─ covered & candidate_rules == [] ─► STOP  └─ any triggered=false & iter = 3 ─► EMIT (residual gap) ──► STOP
```
Tuần tự nghiêm ngặt. Không skip stage. **Iteration counter**: khởi tạo `iter=1` tại VERIFY lần đầu; mỗi lần VERIFY fail loop về DESIGN tăng `iter`. EMIT chỉ xảy ra sau VERIFY — hoặc pass (all triggered), hoặc terminal (iter=3).
Metadata fail (gate `no-deny`/`metadata-tiered`/`transforms-mandatory`/`id-placeholder` không thỏa) phát hiện cuối SYNTHESIZE → fix trong SYNTHESIZE, không thoát khỏi SYNTHESIZE → VERIFY.
...
Sau khi chạy `parse_verdict.py`, đọc `coverage` trong `author-context.json` (KHÔNG tự đọc verdict.json để phán):
...
1. **Parse verdict bằng script** — KHÔNG `Read` cả `verdict.json` (nhiều noise: raw `matched_rules` + full tag arrays + protocol/SQLi FP collateral). Script project về whitelist + tính sẵn tín hiệu probe:
   ...
   ```
   **Read `out/<id>/author-context.json`** (đã gọn). Guard gate `input-guard` theo `coverage` + `candidate_rules`.
   **Read `out/<id>/extended-requests.json`** nếu tồn tại (output của crs-variant-gen). Đọc `labels[]` + `meta[]` để biết tên và rationale mỗi variant — đây là **design target**: rule phải bao phủ mọi request trong này. Không tồn tại → `verify_rule.py` sẽ báo lỗi; chạy crs-variant-gen trước (passthrough cũng đủ).
...
1. **INGEST** — chạy `parse_verdict.py` → Read `author-context.json` (KHÔNG đọc verdict.json); guard `input-guard` theo `coverage`+`candidate_rules`; đọc classification / payload_samples / scope_signal / pl_gap / candidate_rules / already_covered_by; Read `extended-requests.json` nếu có (labels + meta = design target); Read Nuclei template; chốt target_file (từ `target_file_hint`, override theo family) + phase (từ scope).
...
5. `extended-requests.json` không tồn tại → `verify_rule.py` exit lỗi ngay → báo user chạy crs-variant-gen trước (kể cả passthrough mode vẫn tạo file với chỉ PoC). KHÔNG bỏ qua VERIFY.
