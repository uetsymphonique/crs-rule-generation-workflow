...
| 3 parse | `probe-raw.json` → `probe.json` (rồi **auto-xoá** `probe-raw.json`) | `python .claude/skills/crs-retrieve-analyze/tools/parse_probe.py out/<id>/probe-raw.json out/<id>/probe.json` *(thêm `--keep-raw` để giữ raw)* |
| 4 read | `probe.json` → (model) | **Read** `out/<id>/probe.json` — `probe-raw.json` đã bị dọn |
...
2. **Bắt đúng exploit** — rule fire trên chính request thực thi exploit (không phải discover/setup request) tại nơi payload thật sự đi vào, và detection đủ **generic** để trigger bởi pattern exploit (không trùng hợp token vô tình). Đối chiếu `matched_var` (engine trả, dạng SecLang `ARGS:uid`…) với param/location đã ghi ở `injection_point` (plain) / `injection_slot` (SecLang) — cùng chỗ thì khớp. Covered → set `injection_slot` khớp `matched_var` của root-cause rule.
...
- **`out/<id>/analysis.json`** — model write, **chỉ judgment** (classification, payload_samples, root-cause ids + reason, recommendation/candidate_rules). Pure staging: `assemble_verdict.py` **auto-xoá** sau khi `verdict.json` ghi xong (abort thì giữ; `--keep-analysis` để soi).
...
```json
{
  ...
  "scope_gate": {
    ...
    "rationale": "<1-2 câu: vì sao mỗi G ra như vậy + kết luận; vd missing-auth không content token, cần biết endpoint protected (app-state), exploit là vắng Authorization, nhưng path+missing-header expressible → virtual-patch-only>"
  },
  "note": null
}
```
...
- **`scope_gate`** (SCOPE-GATE, xem SKILL.md): **bắt buộc khi not-covered**, **BỎ khi covered** (G0 route covered ra ngoài gate — script abort nếu covered mà vẫn có `scope_gate`). `decision` ∈ {`in-scope`, `virtual-patch-only`, `out-of-scope-structural`} (script validate enum). `virtual-patch-only`/`out-of-scope-structural` ⇒ GEN-VARIANTS **không** spawn (quyết ở SCOPE-GATE, trước assemble). Trace này chỉ để đọc/audit, script không suy gì từ nó ngoài passthrough + validate.
...
```json
{
  "template_id": "duomicms-sql-injection",
  "template_path": "<đường dẫn file .yaml>",
  "classification": { "families": ["sqli"], "injection_point": "query param `uid` ...", "injection_slot": "ARGS_GET:uid", "severity": "critical", "protocol": "http", "confidence": "high", "cwe_hint": "CWE-89" },
  "payload_samples": [
...
- Ca **not-covered**: `root_causes: null`, `candidate_rules` = list từ analysis (đã rank theo độ liên quan), `probe.matched_rules[].root_cause` toàn `false`, **`scope_gate`** = trace G0–G4 (script passthrough nguyên block sau khi validate `decision` enum). Covered → `scope_gate` không xuất hiện.

### Quy tắc populate (model chỉ write analysis.json; script lo phần còn lại)
**Model write (analysis.json):**
- `classification.injection_point` **luôn** giữ, ghi **plain** (request exploit nào + param/header/body mang payload). Gate root-cause cần nó để đối chiếu với `matched_var`; nhánh not-covered cần nó cho Rule Designer (Action B), vì khi KHÔNG rule nào fire thì `matched_var` rỗng → injection_point là record duy nhất về điểm tiêm (dạng người đọc).
- `classification.injection_slot` **luôn** giữ, ghi dạng **ModSec request-variable** của request `exploit_index` (vd `REQUEST_HEADERS:Authorization`, `ARGS_GET:uid`, `ARGS_POST:cmd`, `REQUEST_BODY`, `REQUEST_COOKIES:PHPSESSID`). Đây là vector chuẩn chia sẻ xuống Stage 2 (variant-gen craft trong slot này — `build_extended.py::resolve_slot` validate; rule-author scope rule vào đúng biến này). Covered → phải khớp `matched_var` của root-cause rule. Var được phép + map sang slot vật lý: `crs-variant-gen/reference.md ##SCHEMA` (header/cookie cần `:Name`; `ARGS`/`ARGS_NAMES` mơ hồ → dùng `ARGS_GET`/`ARGS_POST`; `REQUEST_LINE`/`REQUEST_PROTOCOL`/`REQUEST_METHOD`/`FULL_REQUEST` không phải slot đơn — tách sub-vector cụ thể).
...
- `scope_gate` (bắt buộc khi not-covered) — block trace G0–G4 + `decision` + `rationale` từ SCOPE-GATE. Model điền sau khi adjudication ra not-covered, trước GEN-VARIANTS. Covered ⇒ BỎ (script abort nếu có).
